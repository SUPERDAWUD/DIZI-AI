...
def train():
    ...
    model = CustomModel(input_dim=32, hidden_dim=128, output_dim=32).to(device)
    ...
    for epoch in range(epochs):
        total_loss = 0
        for inp, tgt in data:
            inp_tensor = text_to_tensor(inp, vocab).to(device)
            tgt_tensor = text_to_tensor(tgt, vocab).to(device)
            optimizer.zero_grad()
